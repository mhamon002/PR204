...
*/
    ...
    for (i = 0; i < num_procs; i++) {
        memset(buf, '\0', 256);
        sprintf(buf, "%d\n",num_procs);
        sendline(proc_array[i].connect_info.sock_fd, buf, strlen(buf));
    }
    ...
    for(i = 0; i < num_procs; i++){
      for(j = 0; j < num_procs; j++){
        ...
        sendline(proc_array[i].connect_info.sock_fd, buf, strlen(buf)); // nom des machines
        memset(buf,0, 256);
        sprintf(buf, "%d", proc_array[j].connect_info.addr_in->sin_port);
        sendline(proc_array[i].connect_info.sock_fd, buf, strlen(buf)); // numéro ports
      }
    }
